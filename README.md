...
### 4. Vercel デプロイ時の Git コマンド例

変更を反映してデプロイするには、以下を実行してから GitHub にプッシュします。

**リポジトリのルートが親フォルダ（例: Cursor）の場合:**

```powershell
cd "c:\Users\あなたのユーザー名\OneDrive\ドキュメント\Apps\Cursor"
git add .
git status
git commit -m "メッセージ（例: 音声入力のみモード対応）"
git push origin main
```

**リポジトリのルートが english-learning の場合:**

```powershell
cd "c:\Users\あなたのユーザー名\OneDrive\ドキュメント\Apps\Cursor\english-learning"
git add .
git status
git commit -m "メッセージ（例: 音声入力のみモード対応）"
git push origin main
...
プッシュ後、Vercel が自動でビルド・デプロイします。ブランチ名が `main` でない場合は `git branch` で確認し、`main` の部分を実際のブランチ名に置き換えてください。
...
`lib/featureFlags.ts` の **`TEXT_INPUT_ENABLED`** で、テキストエリアの表示を切り替えられます。

- **`false`**（既定）: 音声入力のみ。テキストエリアは非表示で、認識結果だけ表示。送信後は音声入力を自動でオフにし、次の質問ではユーザーが自分でマイクをオンにする。
- **`true`**: 音声入力と文字入力の両方を使用可能（従来どおり）。

将来的に文字入力を使う場合は、`TEXT_INPUT_ENABLED` を `true` に変更してください。
