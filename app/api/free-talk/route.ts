...
import {
  getPartnerReply,
  getPartnerReplyWithCustomSetting,
} from "@/lib/ai/freeTalkService";
...
export async function POST(request: Request) {
  try {
    ...
    const customSetting = body?.customSetting?.trim();
    ...
    if (customSetting && typeof customSetting === "string") {
      const { partnerReply, optionalHint } = await getPartnerReplyWithCustomSetting(
        customSetting,
        history,
        userInput
      );
    ...
    if (!scenarioId || typeof scenarioId !== "string") {
      return NextResponse.json(
        { ok: false, error: "scenarioId or customSetting is required" },
        { status: 400 }
      );
    }
